...
- Docs site theme switched to `jekyll-theme-midnight` (dark) from `jekyll-theme-minimal`. Considered `jekyll-theme-chirpy` first, but it needs a GitHub Actions build and a blog-post content structure (`_posts/`, tags, archive) this docs site doesn't have — a one-line `theme:` swap to one of GitHub Pages' own dark-by-default themes gets the actual ask (dark mode) without any of that.
