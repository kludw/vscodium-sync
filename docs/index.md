...
VSCodium Sync keeps your global `settings.json`, `keybindings.json`, and installed extensions in sync across machines using a private GitHub gist. It's plug and play: install it, sign in to GitHub once, and it stays in sync in the background — no config file, no manual "sync now" step required (though one's available).

## What it does

- **Two-way sync** of your global `settings.json`, `keybindings.json`, and installed extensions through a private GitHub gist.
...
- **Status bar indicator** with a menu to sync on demand, open the gist, jump to `settings.json` or `keybindings.json`, or view the log.
- **Notifications** whenever settings, keybindings, or extensions actually change — with a diff view for settings and keybindings.

## Scope

`settings.json`, `keybindings.json` (both global user files), and the installed-extensions list are synced. Snippets aren't yet — see [Architecture](./architecture.html) for how that could extend.
