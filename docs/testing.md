...
- `src/sync/engine.ts` — the full three-item sync orchestration: linking a new gist by discovery vs. creation, seeding a keybindings or extensions file onto a gist that predates it, steady-state push/pull/none for settings, keybindings, and extensions independently, the content-equality guard that prevents the shared gist timestamp from causing a spurious push or pull, and re-linking when a previously-linked gist 404s versus re-throwing any other error — with `global.fetch` mocked (via `bun:test`'s `spyOn`) and every collaborator injected as a fake.
...
- `src/github/client.ts` — every gist API call (`findSyncGist`, `createSyncGist`, `getGist`, `updateSyncGist`) across all three synced files, including the non-2xx error path, against a mocked `fetch`.
- `src/settings/path.ts` — path resolution for both `settings.json` and `keybindings.json`, across `darwin`, `win32`, and the Linux/XDG fallback.
